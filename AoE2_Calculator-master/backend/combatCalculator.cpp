...
int combatCalculator::generateD6DieInput()
{
  ...
  std::mt19937 gen(rd()); // Initialize the Mersenne Twister pseudo-random number generator
  std::uniform_int_distribution<> dis(1, 6); // Make the pseudo-random numbers uniformly distributed in range (1, 6)
...
void combatCalculator::checkD6DiceSimulator()
{
  ...
  for (int i = 0; i < 50; i++) {
    ...
    std::cout << "die roll: " << i << " has a d6 value of: " << d6DieRoll << "<br>";
  }
}
...
void combatCalculator::checkIfItCanBeHealed()
{
  ...
  // If p1AssistingMonkParticipant has less units than it started with and there are heals available to it, heal it
  ...
  // If p2AssistingMonkParticipant has less units than it started with and there are heals available to it, heal it
...
void combatCalculator::checkIfDead()
{
  ...
  if ( (p1BattleParticipant.entityQuantity <= 0) && (p1BattleParticipant.armorClass[1] == false) ) {
    if (p1AssistingMonkParticipant.entityQuantity > 0) {
      std::cout << player1Name << " has a monk that has prevented all of their units from dying<br>";
      p1BattleParticipant.entityQuantity++;
    }
    else {
  ...
  if ( (p2BattleParticipant.entityQuantity <= 0) && (p2BattleParticipant.armorClass[1] == false) ) {
    if (p2AssistingMonkParticipant.entityQuantity > 0) {
      std::cout << player2Name << " has a monk that has prevented all of their units from dying<br>";
      p2BattleParticipant.entityQuantity++;
    }
    else {
      aDeathHasOccured = true;
...
void combatCalculator::checkRemainingDamage( // @Kory todo: check if this is working as intended
  int* inputP1Events,
  int* inputP2Events)
{
  ...
  std::string inputRunConditions)
{
  ...
  /* P1 stuff */
  bool p1ArcherActivated = false; //  Store if a ranged unit from P1 did something
  ...
  bool p1RangedUnitCanAttackP2Building = false; // Store if archers can attack buildings
  // @Kory todo: Add a new event that allows archers to be able to fight buildings
  bool p1RangedUnitCanAttackP2Cavalry = false; //  Store if archers can attack cavalry
  ...
  /* P2 stuff */
  bool p2ArcherActivated = false; //  Track if a ranged unit from either players did something
  ...
  for (int i = 0; i < inputRunTimes; i++) {
    ...
    if ((aDeathHasOccured == false) && (isRetreating != "1")) { // Proceed if neither player has retreated and no deaths have occured
  ...
  if((inputRunConditions == "p1_calculations") || (inputRunConditions == "p1_and_p2_calculations")){
    ...
    if (isP1FightingBuilding == true) {
      ...
      if(p1RangedUnitCanAttackP2Building == true){
            ...
            p2BuildingDamage = (p1BattleParticipant.rangedDamage * p1BattleParticipant.entityQuantity);
            p2BuildingDamage += roundAttackModifiersP1;
            p2BuildingDamage -= (p2BattleParticipant.entityHealth % roundDownMultiple);
    ...
    // Perform archer vs unit combat if an archer is fighting a unit
      ...
      if (
        ...
        (p2BattleParticipant.armorClass[4] == true) && // Cavalry armor class
        (p1RangedUnitCanAttackP2Cavalry == false)
            ...
            p1RangedDamageDealt = p1BattleParticipant.rangedDamage * p1BattleParticipant.entityQuantity;
           ...
           // Behavior: Make ranged combat attacks for P1 ineffective if this event or shots in the back event is active for P2
    if  ( (inputP2Events[15] == 1) || (protectP2FromTakingDamageDueToShotsInTheBack == true) ) {
  ...
  if (p1ArcherActivated == true) {
    ...
    if ( (isP1FightingBuilding == true) && (p2BuildingDamage != 0) && (p1RangedUnitCanAttackP2Building == true) ) {
            p2BattleParticipant.entityHealth -= p2BuildingDamage; // Behaviour: Decrease the building HP
            if (p2BattleParticipant.entityHealth <= 0) {
            p1PointsGained = p2BattleParticipant.pointValue; // Behaviour: Set the points lost value to the buildings point value
            }
            else if (p2BattleParticipant.entityHealth > 0) {
            p1PointsGained = 0; // Behaviour: if the HP of the building is > 0 then do not keep the set the point value
            }
  ...
  if (aDeathHasOccured == false) {
    ...
    if ((isRetreating == "1") && (inputP1Events[29] == 1) || (inputP2Events[29] == 1)) {
            ...
            std::cout << "Did the conditions of \"Shots In The Back (Briton)\" get "
            ...
            // Behavior: Have it so no ranged damage is taken for the player with the event in question
